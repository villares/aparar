...
def lista_imagens(dir=None):
    """
    Devolve uma lista de Paths dos arquivos de imagem
    na pasta `dir` ou na pasta /data/ do diretório corrente.
    """
    ...
    # FIX: was Path(dir) which ignored the data_path fallback and crashed when dir=None
    data_path = Path(dir) if dir is not None else Path.cwd() / 'data'
    try:
        p_list = [fp for fp in data_path.iterdir()
...
def carrega_pranchas():
    # Operação normal de adicionar_imagens() é via callback disparado por select_folder()
    ...
    # adicionar_imagens(Path("/home/villares/Área de Trabalho/APARAR/Pranchas para teste"))
...
def salva_sessao():
    with open(pr.Prancha.path_sessao / NOME_ARQ_SESSAO, "wb") as file:
        ...
        sessao = (pr.Prancha.pranchas, (Area.categorias, Area.tags), pr.Prancha.screen_height)
        pickle.dump(sessao, file)
    mensagem = "sessão salva em …" + str(pr.Prancha.path_sessao)[-40:]  # FIX: unicode() → str()
...
def salva_png():
    ...
    temp_path = Path.cwd() / 'data' / 'temp.png'
    save_frame(temp_path)
    ...
    png.end_draw()
    if modo_diagrama:
        salva_legenda_diagrama(path_folder)  # FIX: was undefined `path`, must be `path_folder`
    pr.Prancha.avisos("Imagem salva: {}".format(nome_arquivo))

def salva_legenda_diagrama(path_folder):
    ...
    png.save(path_folder / "legenda.png")
    png.end_draw()
